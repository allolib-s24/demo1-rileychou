...
//   gam::Cos
  ...
  void init() override {
    ...
    createInternalTriggerParameter("vibDepth", 0.5, 0.0, 1.0);
    createInternalTriggerParameter("vibFreq", 2000.0, 0.0, 4000.0);
  }
  ...
  void onProcess(AudioIOData &io) override {
    ...
    // mVib.freq(getInternalParameterValue("vibFreq"));
    // float freq = getInternalParameterValue("frequency");
    // const float twelthRootOf2 = 1.05946309436;
    // float vibDepth = getInternalParameterValue("vibDepth");
    // float scaledVibDepth = pow(twelthRootOf2 - 1, vibDepth);
    // float vibMult = 1 + (mVib() * scaledVibDepth);

    mVib.freq(getInternalParameterValue("vibFreq")); // between 10 and 20
    float freq = getInternalParameterValue("frequency"); // the note im playing
    // 12th root of 2
    float twelthRoot2 = 1.0594630943592953;
    float vibDepth = getInternalParameterValue("vibDepth");

    float scaledVibDepth = pow(twelthRoot2, (vibDepth));
    // float vibDepthHertz = (freq * scaledVibDepth);
    float delta = (mVib() * (scaledVibDepth - 1)); // 

    std::cout << "mVib(): " << mVib() << "\n";
    // std::cout << "vibMult: " << vibMult << std::endl;
    std::cout << mOsc() << std::endl;

    // mOsc.freq(getInternalParameterValue("frequency")); // freq times vibMult
    // mOsc.freq(freq + (vibDepthHertz * mVib()));
    mOsc.freq(freq * (delta + 1));
